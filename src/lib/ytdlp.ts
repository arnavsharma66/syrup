...
const SYRUP_DIR = path.join(os.homedir(), '.syrup', 'bin')
...
export async function ensureYtDlp(onStatus: (message: string) => void, signal?: AbortSignal): Promise<string> {
  ...
  const local = path.join(SYRUP_DIR, process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp')
  if (await commandWorks(local, ['--version'])) return local

  onStatus('first run: fetching yt-dlp…')
  await fs.mkdir(SYRUP_DIR, {recursive: true})
...
/**
 * Update the bundled yt-dlp binary by re-downloading the latest from GitHub releases.
 * If the current binary is a system install (on PATH), it downloads a fresh copy
 * to ~/.syrup/bin/ instead.
 */
export async function updateYtDlp(currentPath: string, onStatus: (message: string) => void): Promise<void> {
  const local = path.join(SYRUP_DIR, process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp')

  // get current version
  let oldVersion = 'unknown'
  try {
    oldVersion = await new Promise<string>((resolve, reject) => {
      const child = spawn(currentPath, ['--version'])
      let out = ''
    ...
    // couldn't get version, proceed anyway
  }

  onStatus(`updating yt-dlp (current: ${oldVersion})…`)
  await fs.mkdir(SYRUP_DIR, {recursive: true})
  ...
  try {
    newVersion = await new Promise<string>((resolve, reject) => {
      const child = spawn(local, ['--version'])
      let out = ''
      child.stdout.on('data', chunk => (out += chunk))
      child.on('error', reject)
      child.on('close', () => resolve(out.trim()))
  ...
  onStatus(`✓ yt-dlp updated: ${oldVersion} → ${newVersion}`)
}
...
export async function probe(ytdlp: string, url: string, signal?: AbortSignal): Promise<ProbeResult> {
  ...
  const infoJsonPath = path.join(os.tmpdir(), `syrup-info-${process.pid}-${Date.now()}.json`)
...
/**
 * Probe a URL for playlist info using --flat-playlist. Returns the playlist
 * metadata if the URL is a playlist, or null if it's a single video.
 */
export async function probePlaylist(ytdlp: string, url: string, signal?: AbortSignal): Promise<PlaylistInfo | null> {
  const stdout = await new Promise<string>((resolve, reject) => {
  ...
  const entries: PlaylistEntry[] = (data.entries as Array<Record<string, unknown>>)
    .filter(e => typeof e.url === 'string' || typeof e.webpage_url === 'string')
    .map(e => ({
      title: (e.title as string) ?? 'Untitled',
      url: (e.webpage_url as string) ?? (e.url as string),
