...
test('--mp3 flag requires a url', () => {
  assert.match(parseArgs(['--mp3']).error ?? '', /requires a url/)
  assert.deepEqual(parseArgs(['--mp3', 'https://example.com/video']), {
    help: false,
    version: false,
    mp3: true,
    initialUrl: 'https://example.com/video',
...
test('--best and --mp3 are mutually exclusive', () => {
  assert.match(parseArgs(['--best', '--mp3', 'https://example.com']).error ?? '', /cannot be used together/)
})

test('-o / --output parses a directory path', () => {
  assert.deepEqual(parseArgs(['-o', '/tmp', 'https://example.com/video']), {
    help: false,
    version: false,
    outDir: '/tmp',
    initialUrl: 'https://example.com/video',
  })
  assert.deepEqual(parseArgs(['--output=/tmp/vids', 'https://example.com/video']), {
    help: false,
    version: false,
    outDir: '/tmp/vids',
    initialUrl: 'https://example.com/video',
