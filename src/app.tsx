...
const SYRUP_BUTTON = 'syrup'
const DONE_LABEL = '↵ syrup another'
const TAGLINE = 'syrup any video. paste. syrup. done.'
...
export type Outcome = {filepath?: string; filepaths?: string[]}
...
type Phase =
  ...
  | {name: 'playlist'; playlist: PlaylistInfo}
  | {
      name: 'batch-downloading'
  ...
  | {name: 'batch-done'; filepaths: string[]}
...
function AppContent({
  ...
  const effectiveOutDir = outDirProp ?? OUT_DIR
  const theme = useTheme()
  const {exit} = useApp()
  ...
  const startProbe = useCallback(async (targetUrl: string) => {
    ...
    try {
      ...
      // check if the URL is a playlist first
      setPhase({name: 'probing', status: 'checking for playlist…'})
      const playlistInfo = await probePlaylist(ytdlp, targetUrl, controller.signal)
      if (controller.signal.aborted) return

      if (playlistInfo && playlistInfo.entries.length > 1) {
        setPhase({name: 'playlist', playlist: playlistInfo})
        return
      }
      ...
      setInfo(videoInfo)
      const builtChoices = buildChoices(videoInfo)
      setChoices(builtChoices)
      highlightRef.current = 0
      if (scriptMode) {
        // auto-select: --best picks first video choice, --mp3 picks the audio choice
        const autoChoice = scriptMode === 'mp3'
          ? builtChoices.find(c => c.kind === 'audio') ?? builtChoices[builtChoices.length - 1]
          : builtChoices[0]
        if (autoChoice) {
          setPhase({name: 'picking'}) // brief transition
          // trigger download immediately via handlePick equivalent
          const idx = builtChoices.indexOf(autoChoice)
          setTimeout(() => handlePick({value: idx}, builtChoices), 0)
          return
        }
      }
  ...
  useInput(
    (input, key) => {
      ...
      if (key.escape && (phase.name === 'picking' || phase.name === 'error' || phase.name === 'done' || phase.name === 'playlist' || phase.name === 'batch-done')) resetToInput()
      if (key.escape && (phase.name === 'probing' || phase.name === 'downloading' || phase.name === 'batch-downloading')) cancelRun()
      if (key.return && (phase.name === 'error' || phase.name === 'done' || phase.name === 'batch-done')) resetToInput()
      if (key.return && phase.name === 'playlist') handlePlaylistDownload(phase.playlist)
    },
  ...
  const handlePick = (item: {value: number}, overrideChoices?: DownloadChoice[]) => {
    const activeChoices = overrideChoices ?? choices
    const choice = activeChoices[item.value]
    if (!choice) return
    const controller = new AbortController()
    abortRef.current = controller
    ...
    void (async () => {
      ...
      try {
        ...
        const base = {ytdlp: ytdlpRef.current, ffmpegLocation, url, choice, outDir: effectiveOutDir}
        let filepath: string
        try {
  ...
  const handlePlaylistDownload = (playlist: PlaylistInfo) => {
    // download best quality for all entries
    const bestChoice: DownloadChoice = {
      kind: 'video',
    ...
    setPhase({
      ...
      choice: bestChoice,
      currentIndex: 0,
      currentTitle: playlist.entries[0]?.title ?? '',
      processing: false,
      completed: [],
    })
    void (async () => {
      ...
      try {
        ...
        for (let i = 0; i < playlist.entries.length; i++) {
          ...
          setPhase(prev => prev.name === 'batch-downloading' ? {
            ...prev,
            currentIndex: i,
          ...
          const handlers = {
            onProgress: (progress: DownloadProgress) =>
              setPhase(prev => (prev.name === 'batch-downloading' ? {...prev, progress, processing: false} : prev)),
            onProcessing: () =>
              setPhase(prev => (prev.name === 'batch-downloading' ? {...prev, processing: true} : prev)),
          }
          try {
            const base = {ytdlp: ytdlpRef.current, ffmpegLocation, url: entry.url, choice: bestChoice, outDir: effectiveOutDir}
            const filepath = await download(base, handlers, controller.signal)
            completed.push(filepath)
            setPhase(prev => prev.name === 'batch-downloading' ? {...prev, completed: [...completed]} : prev)
          } catch (error) {
            if (controller.signal.aborted) return
            // skip failed entries, continue with next
            completed.push(`✗ ${entry.title}`)
          }
        }
        onOutcome({filepaths: completed.filter(f => !f.startsWith('✗'))})
        setHistory(addToHistory(url))
        setPhase({name: 'batch-done', filepaths: completed})
      } catch (error) {
        if (controller.signal.aborted) return
        setPhase({name: 'error', message: error instanceof Error ? error.message : String(error)})
      }
  ...
  if (phase.name === 'input') {
    hints.push(['syrup -o <dir>', 'default folder'])
    if (history.length > 0) {
  ...
  const hintAction = (key: string): (() => void) | undefined => {
    ...
    if (key === 'esc') return phase.name === 'probing' || phase.name === 'downloading' || phase.name === 'batch-downloading' ? cancelRun : resetToInput
    if (key === '↵') {
      if (phase.name === 'input') return () => handleUrlSubmit(urlInput)
      if (phase.name === 'picking') return () => handlePick({value: highlightRef.current})
      if (phase.name === 'playlist') return () => handlePlaylistDownload(phase.playlist)
  ...
  if (phase.name === 'input') {
    ...
    clickTargets.push({match: `  ${SYRUP_BUTTON}  `, padY: 1, action: () => handleUrlSubmit(urlInput)})
    clickTargets.push({match: 'paste clipboard', padX: 3, padY: 1, action: () => {
  ...
  if (phase.name === 'playlist') {
    clickTargets.push({match: 'download all', padX: 4, padY: 1, action: () => handlePlaylistDownload(phase.playlist)})
  }
  ...
  return (
    <FullScreen>
      ...
      <Text color={theme.gray} dimColor={theme.dimSecondary}>youtube · x · instagram · threads · tiktok · +2000 more</Text>
      <Gap />

      {phase.name === 'input' && (
        <Box flexDirection="column" alignItems="center">
          <FramedInput title="Paste a link" width={boxWidth} button={SYRUP_BUTTON}>
            <TextInput
              value={urlInput}
          ...
          {phase.warning ? (
            ...
            <Text color={theme.gray} dimColor={theme.dimSecondary}>copied link found — ⇥ to paste</Text>
          ) : clipboardAccepted ? (
            <Text color={theme.gray} dimColor={theme.dimSecondary}>copied link pasted — ↵ to syrup it</Text>
          ) : null}
          <Gap />
          <Box
            borderStyle="round"
            ...
            paddingX={3}
          >
            <Text bold color={theme.primary}>paste clipboard</Text>
          </Box>
          <Gap />
          <Text color={theme.gray} dimColor={theme.dimSecondary}>
            default folder: {shortenPath(effectiveOutDir, os.homedir(), 40)}
          </Text>
        </Box>
      )}
      ...
      {phase.name === 'probing' && (
        <Box flexDirection="column" alignItems="center">
          <FramedInput title={platform ? platform.label : 'Paste a link'} width={boxWidth} button={SYRUP_BUTTON} buttonDim>
      ...
      {phase.name === 'playlist' && (
        <Box flexDirection="column" alignItems="center" width={contentWidth}>
          <Text bold color={theme.primary}>Playlist: {phase.playlist.title}</Text>
          <Text color={theme.gray} dimColor={theme.dimSecondary}>Found {phase.playlist.entries.length} videos</Text>
          <Gap />
          <Box flexDirection="column" borderStyle="round" borderColor={theme.gray} paddingX={2} width={Math.min(contentWidth, 60)}>
            {phase.playlist.entries.slice(0, 5).map((entry, idx) => (
              <Text key={idx} color={theme.primary}>
                {idx + 1}. {truncate(entry.title, Math.max(10, contentWidth - 10))}
              </Text>
            ))}
            {phase.playlist.entries.length > 5 && (
              <Text color={theme.gray} dimColor={theme.dimSecondary}>
                ... and {phase.playlist.entries.length - 5} more videos
              </Text>
            )}
          </Box>
          <Gap />
          <Box
            ...
            paddingX={3}
          >
            <Text bold color={theme.primary}>download all</Text>
          </Box>
        </Box>
      )}
      ...
      {phase.name === 'batch-downloading' && (
        <Box flexDirection="column" alignItems="center">
          <Text bold color={theme.primary}>
            Downloading playlist ({phase.currentIndex + 1}/{phase.playlist.entries.length})
          </Text>
          <Text color={theme.gray} dimColor={theme.dimSecondary}>
            {truncate(phase.currentTitle, Math.max(10, columns - 10))}
          </Text>
          <Gap />
          {phase.processing ? (
            <>
            ...
            <>
              ...
              <Text>
                ...
                <Text color={theme.gray} dimColor={theme.dimSecondary}> starting download…</Text>
              </Text>
            </>
          )}
        </Box>
      ...
      {phase.name === 'done' && (
        <Box flexDirection="column" alignItems="center">
          <Text>
            <Text bold color={theme.primary}>✓ syruped! </Text>
      ...
      {phase.name === 'batch-done' && (
        <Box flexDirection="column" alignItems="center">
          <Text>
            <Text bold color={theme.primary}>✓ Playlist download complete! </Text>
          </Text>
          <Text color={theme.gray} dimColor={theme.dimSecondary}>Saved {phase.filepaths.length} files to {shortenPath(effectiveOutDir, os.homedir(), 40)}</Text>
          <Gap />
          <Box
            borderStyle="round"
            borderColor={theme.gray}
